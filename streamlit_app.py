...
# Reading CSV File
df = pd.read_csv("clean_spaceship_data.csv")
...
def web_app():
    ...
    # Feature Inputs
    with col2:
        home_planet = st.selectbox(label="Select the Home Planet", options=df["HomePlanet"].unique(), placeholder="Choose the Home Planet", index=None)

        destination = st.selectbox(label="Select the Destination Planet", options=df["Destination"].unique(), placeholder="Choose the Destination Planet", index=None)
    ...
    with col3:
        ...
        room_service = st.selectbox(label="Select the Room Service Amount", options=df[subset_cols[0]].round(4), placeholder="Select the Amount...", index=None, help="These inputs are Standardized by using StandardScaler")

        food_court = st.selectbox(label="Select the Food Cort Amount Spend", options=df[subset_cols[1]].round(4), placeholder="Select the Amount...", index=None, help="These inputs are Standardized by using StandardScaler")

        spa = st.selectbox(label="Select the Spa Service Amount Spend", options=df[subset_cols[2]].round(4), placeholder="Select the Amount...", index=None, help="These inputs are Standardized by using StandardScaler")

        vr_deck = st.selectbox(label="Select the VRDeck Amount Spend", options=df[subset_cols[3]].round(4), placeholder="Select the Amount...", index=None, help="These inputs are Standardized by using StandardScaler")

        shopping_mall = st.selectbox(label="Select the Shopping Amount Spend", options=df[subset_cols[4]].round(4), placeholder="Select the Amount...", index=None, help="These inputs are Standardized by using StandardScaler")
    ...
    model_features = ['CryoSleep', 'Age', 'VIP', 'Earth', 'Europa', 'Mars', '55 Cancri e', 'PSO J318.5-22', 'TRAPPIST-1e']
...
st.sidebar.image("icon.png", caption="Spaceship Titanic Classification", use_container_width=True)
